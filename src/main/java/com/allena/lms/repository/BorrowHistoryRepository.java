...
public class BorrowHistoryRepository {
    ...
    public boolean borrowBook(BorrowHistoryDTO borrowHistory) throws Exception {
       return borrowHistoryJdbcTemplate
               .update(
                       "INSERT INTO TBL_BORROW_HISTORIES (id,student_id,book_id,issue_date) VALUES (?,?,?,?)",
                       new Object[]{borrowHistory.getId(),borrowHistory.getStudentId(),borrowHistory.getBookId(),borrowHistory.getIssueDate()}
               ) > 0;
    }

    public boolean returnBook(BorrowHistoryDTO borrowHistoryDTO) throws Exception {
        return borrowHistoryJdbcTemplate.update(
                "UPDATE TBL_BORROW_HISTORIES SET return_date=? WHERE book_id=? AND student_id=? AND return_date IS NULL",
                new Object[]{borrowHistoryDTO.getReturnDate(),borrowHistoryDTO.getBookId(), borrowHistoryDTO.getStudentId()}
        ) > 0;
    }
}